...
console.log("Le nombre smarties la 1ere boite est de"), console.log(totalSmarties(boxes[0]));

function totalSmartiesPerBox(tab){
    for (let i=0;i<tab.length;i++){
        console.log(totalSmarties(boxes[i]));
    }
}
console.log("Voici le nombre de smarties par boites:"), totalSmartiesPerBox(boxes);

// Quel est le nombre minimum de Smarties® dans une boîte ?
function minimumCount(tab){
    let minicount=totalSmarties(tab[0]);
    for (let i=0;i<tab.length;i++){
        let totalActuel=totalSmarties(tab[i])
        if (totalActuel < minicount){
            minicount=totalSmarties(tab[i]);
        }
    }
    return minicount;
}
console.log("Le nombre minimum de smarties dans une boîte est de"),console.log(minimumCount(boxes));

// Et le nombre maximum ?
function maximumCount(tab){
    let maxicount=totalSmarties(tab[0]);
    for (let i=0;i<tab.length;i++){
        let totalActuel=totalSmarties(tab[i])
        if (totalActuel > maxicount){
            maxicount=totalSmarties(tab[i]);
        }
    }
    return maxicount;
}
console.log("Le nombre maximum de smarties dans une boîte est de"),console.log(maximumCount(boxes));
